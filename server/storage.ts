...
export interface IStorage {
  ...
  listIntakeRecords(platformUserId?: string): Promise<IntakeRecord[]>;
...
export class DbStorage implements IStorage {
  ...
  async listIntakeRecords(platformUserId?: string): Promise<IntakeRecord[]> {
    if (platformUserId) {
      return db.select().from(intakeRecords)
        .where(or(
          eq(intakeRecords.tspContactAssigned, platformUserId),
          eq(intakeRecords.tspContact, platformUserId),
        ))
